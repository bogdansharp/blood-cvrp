...
const makeScenarioReduced = (overrides: Partial<ScenarioReduced> = {}): ScenarioReduced => ({
	id: 42,
	name: 'Scenario 42',
	vehicles_count: 1,
	depots_count: 1,
...
describe('store API/state logic', () => {
	...
	it('loads and stores scenario list', async () => {
		const scenarios = [makeScenarioReduced(), makeScenarioReduced({ id: 43, name: 'Scenario 43' })];
	...
	it('does not submit solve request when no scenario is loaded', async () => {
		...
		await expect(
			store.submitSolveRequest('clarke_wright_savings', 1, 'AUTOMATIC', 'NONE', 'NONE', false)
	...
	it('sends cancel job request', async () => {
		...
		expect(fetchMock).toHaveBeenCalledWith(`${apiBase}/jobs/cancel/123`, { method: 'POST' });
	});
