...
describe('store geometry queue', () => {
	...
	it('fetches missing geometry after loading a solution', async () => {
		const key = makeGeometryKey(depot.lat_e6, depot.lng_e6, customerA.lat_e6, customerA.lng_e6);
	...
	it('does not fetch duplicate route segments more than once', async () => {
		...
		const solution = makeSolution([makeRoute([depot, customerA]), makeRoute([depot, customerA])]);
	...
	it('does not fetch geometry that is already cached', async () => {
		...
		fetchMock.mockResolvedValueOnce(jsonResponse(makeSolution([makeRoute([depot, customerA])])));
