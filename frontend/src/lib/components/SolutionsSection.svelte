<script lang="ts">
	...
	let visibleSolutions = $derived(
		scenarioId === null ? [] : solutions.filter((solution) => solution.scenario_id === scenarioId)
	);
				...
				<li>
					...
					>
						<div class="flex flex-wrap items-center gap-2 text-xs text-slate-600">
							...
							<span
								class="rounded-full bg-slate-200 px-2 py-0.5 text-[11px] font-bold text-slate-800"
							>
