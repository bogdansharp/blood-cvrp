...
<div class="grid gap-3">
    <div class="min-w-0">
        ...
        <input
            class="m-0 wrap-break-words text-lg font-semibold text-slate-900 focus-visible:outline-none"
            value={draft.name}
    ...
    <div class="rounded-xl border border-blue-200 bg-blue-50 p-2 text-sm text-blue-900">
        Click grey hospitals on the map to add customers. Click Select depot”, then click the depot location.
    </div>
            ...
            {#each draft.customers as customer, index}
                <li class="grid gap-2 rounded-xl border border-slate-200 bg-white p-2">
                    ...
                    <input
                        ...
                        oninput={(event) => updateCustomer(index, { demand: Number(event.currentTarget.value) })}
                    />
