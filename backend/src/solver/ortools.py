...
from backend.src.solver.models import (
    INF_VEHICLES,
    Route,
    Solver,
    SolverInput,
...
FIRST_SOLUTION_MAP = {
    ORFirstSolutionStrategy.AUTOMATIC: routing_enums_pb2.FirstSolutionStrategy.AUTOMATIC,
    ORFirstSolutionStrategy.PATH_CHEAPEST_ARC: routing_enums_pb2.FirstSolutionStrategy.PATH_CHEAPEST_ARC,
    ORFirstSolutionStrategy.SAVINGS: routing_enums_pb2.FirstSolutionStrategy.SAVINGS,
    ORFirstSolutionStrategy.PARALLEL_CHEAPEST_INSERTION: routing_enums_pb2.FirstSolutionStrategy.PARALLEL_CHEAPEST_INSERTION,
    ORFirstSolutionStrategy.LOCAL_CHEAPEST_INSERTION: routing_enums_pb2.FirstSolutionStrategy.LOCAL_CHEAPEST_INSERTION,
    ORFirstSolutionStrategy.GLOBAL_CHEAPEST_ARC: routing_enums_pb2.FirstSolutionStrategy.GLOBAL_CHEAPEST_ARC,
}

LOCAL_SEARCH_MAP = {
    ORLocalSearchMetaheuristic.AUTOMATIC: routing_enums_pb2.LocalSearchMetaheuristic.AUTOMATIC,
    ORLocalSearchMetaheuristic.GREEDY_DESCENT: routing_enums_pb2.LocalSearchMetaheuristic.GREEDY_DESCENT,
    ORLocalSearchMetaheuristic.GUIDED_LOCAL_SEARCH: routing_enums_pb2.LocalSearchMetaheuristic.GUIDED_LOCAL_SEARCH,
    ORLocalSearchMetaheuristic.SIMULATED_ANNEALING: routing_enums_pb2.LocalSearchMetaheuristic.SIMULATED_ANNEALING,
    ORLocalSearchMetaheuristic.TABU_SEARCH: routing_enums_pb2.LocalSearchMetaheuristic.TABU_SEARCH,
}
...
class OrToolsSolver(Solver):
    ...
    def _parse_input(self, input: SolverInput) -> None:
        ...
        if max(self._demand) > max(self._capacities):
            raise ValueError(
                "At least one customer demand exceeds all vehicle capacities"
            )
    ...
    def solve(
        ...
        if need_cost_limit_dimension:
            route_cost_limit = (
                ...
                else max(int(round(v)) for row in self._matrix for v in row)
                * (self._n + 1)
            )
        ...
        if local_search != ORLocalSearchMetaheuristic.NONE:
            search_parameters.local_search_metaheuristic = LOCAL_SEARCH_MAP[
                local_search
            ]
